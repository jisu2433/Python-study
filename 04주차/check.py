...
def check_id() :
    while True:
        id = input().split('-')
        if len(id[0]) == 6 or len(id[0]) + len(id[1]) == 13:
            if 0 <= int(id[0][:2]) <= 21:
                ox = input('2000년 이후 출생자 입니까?')
                if (ox == 'o' and 3 <= int(id[1][0]) <= 4) or (ox == 'x' and 1 <= int(id[1][0]) <= 2):
                    print(f'{id[0][:2]}년 {id[0][2:4]}월 {gender[int(id[1][0]) % 2]}')
                    break
                else:
                    print(f'{id[0][:2]}년 {id[0][2:4]}월 {gender[int(id[1][0]) % 2]}')
                    break
